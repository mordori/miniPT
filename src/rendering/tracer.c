...
static inline bool trace_ray(const t_context* ctx, t_path* path, t_pixel* pixel, t_render_mode mode) {
	...
	bool hitSelectedObj = ctx->editor.selected_obj && !(ctx->editor.selected_obj->flags & FLAG_OBJ_HIDDEN_SCENE) &&
		!(is_primary_ray && (ctx->editor.selected_obj->flags & FLAG_OBJ_HIDDEN_CAM)) &&
	...
	if (ctx->scene.env.has_dir_light) {
		...
		hitDirLightObj = !(is_primary_ray && (dir_light->flags & FLAG_OBJ_HIDDEN_CAM)) && hit_object(dir_light, &path->ray, &path->hit);
	}
	bool hitBVH = hit_bvh(ctx->scene.geo.bvh_root_idx, &path->ray, &path->hit, ctx->scene.geo.bvh_nodes);

	if (hitSelectedObj || hitBVH || hitDirLightObj) {
